  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="rounded-lg border bg-white p-12 text-center text-gray-500">Nenhum saque encontrado.</div>
      ) : (
                ...
                return (
                  <tr key={w.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      <button onClick={() => copyPix(w.pix_key)} className="flex items-center gap-1 text-gray-700 hover:text-primary" title="Copiar PIX">
                    ...
                    <td className="px-4 py-3 flex gap-2">
                      {w.status === 'pending' && (
                        <>
                          <button onClick={() => handleApprove(w.id)} disabled={actionLoading} className="rounded bg-green-600 px-2 py-1 text-xs text-white hover:bg-green-700">Aprovar</button>
                          <button onClick={() => setRejectId(w.id)} className="rounded bg-red-600 px-2 py-1 text-xs text-white hover:bg-red-700">Rejeitar</button>
                        </>
                      )}
                      {w.status === 'processing' && (
                        <button onClick={() => handleComplete(w.id)} disabled={actionLoading} className="rounded bg-blue-600 px-2 py-1 text-xs text-white hover:bg-blue-700">Concluir</button>
                      )}
                    </td>
                  </tr>
                );
